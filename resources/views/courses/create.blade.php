<x-loyouts.main title="81-IDUM | Kurs ochish">
    ...
    <section class="container news reveal glass-section">
      <div class="course-create-shell">
        <div class="course-create-info-grid">
          ...
          <aside class="course-create-teacher-card" data-course-teacher-preview>
  ...
  <script>
    (() => {
      const previewData = @json($teacherPreviewData);
      ...
      const fallbackImage = @json(asset('temp/img/how-to-be-teacher-malaysia-feature.png'));
      ...
      const renderTeacher = (teacher) => {
        if (image) {
          image.src = teacher?.image || fallbackImage;
        }
        if (name) {
          name.textContent = teacher?.name || 'Ustoz tanlanmagan';
        }
        if (subject) {
          subject.textContent = teacher?.subject || "Avval ustozni tanlang";
        }
        if (experience) {
          experience.textContent = teacher?.experience_label || '-';
        }
        if (grades) {
          grades.textContent = teacher?.grades || '-';
        }
        if (bio) {
      ...
      if (select) {
        ...
        renderTeacher(previewMap.get(@json($initialTeacherId)) || null);
      }
    })();
  </script>
</x-loyouts.main>

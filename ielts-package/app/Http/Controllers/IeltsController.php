...
class IeltsController extends Controller
{
    ...
    public function index()
    {
        ...
        // TODO: replace with your actual donation-rank accessor, e.g. $user->donationRank()
        $donationRank = method_exists($user, 'donationRank') ? $user->donationRank() : 0;

        $monthlyLimit = match (true) {
            $donationRank >= 2 => null, // unlimited
            $donationRank === 1 => 3,
            default => 1,
        };
        ...
        return view('ielts.index', compact('tests', 'monthlyLimit', 'usedThisMonth', 'donationRank'));
    }
    ...
    public function start(Request $request, IeltsTest $ieltsTest)
    {
        ...
        return DB::transaction(function () use ($user, $ieltsTest) {
    ...
    public function submit(IeltsAttempt $attempt, IeltsGradingService $grading)
    {
        ...
        $user = $attempt->user;
        // TODO: replace with your actual donation-rank accessor
        $donationRank = method_exists($user, 'donationRank') ? $user->donationRank() : 0;
        $detailedFeedback = $donationRank >= 1;

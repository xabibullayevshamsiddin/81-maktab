...
class AdminController extends Controller
{
    ...
    /**
     * Foydalanuvchini oddiy foydalanuvchiga tushirish (har qanday roldan).
     */
    public function demoteUser(User $user)
    {
        ...
        if (! $currentUser || ! $currentUser->isSuperAdmin()) {
            return redirect()->route('user')
                ->with('error', 'Faqat Super Admin bu amalni bajarishi mumkin.')
                ->with('toast_type', 'error');
        }

        if ($user->id === $currentUser->id) {
            return redirect()->route('user')
                ->with('error', "O'zingizni tushira olmaysiz.")
                ->with('toast_type', 'error');
        }

        // Oddiy foydalanuvchi rolini topish
        $userRole = Role::where('name', User::ROLE_USER)->first();
        if (! $userRole) {
            return redirect()->route('user')
                ->with('error', 'Foydalanuvchi roli topilmadi.')
                ->with('toast_type', 'error');
        }

        $oldRole = $user->roleRelation?->label ?? $user->role;
        $user->update(['role_id' => $userRole->id]);

        // Telegram xabar yuborish
        if ($user->telegram_chat_id) {
            $userName = htmlspecialchars($user->buildNameFromParts() ?: $user->name);
            $adminName = htmlspecialchars($currentUser->buildNameFromParts() ?: $currentUser->name);

            $text = "📉 <b>Hisobingiz darajasi o'zgartirildi</b>\n"
                ."━━━━━━━━━━━━━━━━━━━━\n\n"
                ."👤 <b>Foydalanuvchi:</b> {$userName}\n"
                ."👨‍💼 <b>Amal bajargan:</b> {$adminName}\n"
                ."🔄 <b>Eski daraja:</b> {$oldRole}\n"
                ."📋 <b>Yangi daraja:</b> Foydalanuvchi\n\n"
                ."━━━━━━━━━━━━━━━━━━━━\n\n"
                ."⚠️ Barcha maxfiy huquqlaringiz bekor qilindi.";
        ...
        return redirect()->route('user')
            ->with('success', "{$user->name} oddiy foydalanuvchiga tushirildi (oldingi daraja: {$oldRole}).")
            ->with('toast_type', 'warning');
    }
}

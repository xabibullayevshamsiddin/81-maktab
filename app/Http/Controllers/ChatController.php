...
class ChatController extends Controller
{
    ...
    public function messages(Request $request): JsonResponse
    {
        ...
        $data = $messages->map(function (ChatMessage $m) use ($currentUser, $currentUserId, $canModerate) {
            ...
            return [
                ...
                'donor_rank' => $m->user?->isDonor() ? $m->user->donation_rank : null,
                'donor_theme' => ($m->user && ($m->user->donorChatStyle() ?? 'show') !== 'hide') ? ($m->user->effectiveTheme() ?? '') : '',
                'donor_badge' => ($m->user && ($m->user->donorChatStyle() ?? 'show') !== 'hide') ? ($m->user->donorBadgeHtml() ?? '') : '',
                'donor_color' => ($m->user && ($m->user->donorChatStyle() ?? 'show') !== 'hide') ? ($m->user->donorUsernameColor() ?? '') : '',
                'name_font_weight' => ($m->user && ($m->user->donorChatStyle() ?? 'show') !== 'hide') ? $m->user->donorNameFontWeight() : '700',
                'name_font_family' => ($m->user && ($m->user->donorChatStyle() ?? 'show') !== 'hide') ? ($m->user->donorNameFontFamily() ?? '') : '',
                'status_emoji' => $m->user?->donorStatusEmoji() ?? '',
    ...
    public function userPreview(Request $request, User $user): JsonResponse
    {
        ...
        $payload = [
            ...
            'donor_rank' => $user->isDonor() ? $user->donation_rank : null,
            'donor_theme' => $user->effectiveTheme() ?? '',
            'donor_badge' => $user->donorBadgeHtml() ?? '',
            'donor_color' => $user->donorUsernameColor() ?? '',
            'name_font_weight' => $user->donorNameFontWeight(),
            'name_font_family' => $user->donorNameFontFamily() ?? '',
            'show_expiry_badge' => $user->isDonor() ? (string) ($user->show_expiry_badge ?? '1') : '1',
            'donor_expires' => $user->isDonor() ? $user->donation_rank_expires_at?->diffForHumans() : null,
        ];
    ...
    private function buildUserPreviewAdminProfile(User $user, string $roleName, string $roleLabel, int $roleLevel): array
    {
        return [
            ...
            'donor_rank_label' => $user->donorRankLabel(),
            'donor_expires_at' => $user->isDonor() ? $user->donation_rank_expires_at?->format('d.m.Y') : null,
            'show_expiry_badge' => $user->isDonor() ? (string) ($user->show_expiry_badge ?? '1') : '1',
